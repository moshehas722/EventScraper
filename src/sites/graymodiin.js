// Site scraper for grayclub.co.il (Gray Club Modiin / מועדון גריי מודיעין).
//
// WordPress + grayux theme (Kod VeLev). The Modiin location page loads the first
...
//   action=load_more_shows&posts_per_page=N&load_more_shows=6&categorytermid=6
// Response: { status, htmldata, hideButton }
// Each item has: singer-name, date "DD.MM.YYYY", door time "פתיחת דלתות", url /event/{id}/{showId}/.
// categorytermid=6 is the Modiin location term (Yehud=5, Tel Aviv uses another term).

import { fetchPostJson, fetchText } from '../http.js';
import { noopProgress } from '../progress.js';

const ORIGIN = 'https://grayclub.co.il';
const MODIIN_PATH = '/gray-%d7%9e%d7%95%d7%93%d7%99%d7%a2%d7%99%d7%9f/';
const MODIIN_URL = `${ORIGIN}${MODIIN_PATH}`;
const AJAX_URL = `${ORIGIN}/wp-admin/admin-ajax.php`;
const CATEGORY_TERM_ID = '6';
...
/**
 * Fetch and normalize all upcoming Gray Club Modiin events.
...
export async function fetchEvents(progress = noopProgress) {
  const apiHeaders = { Referer: MODIIN_URL, Origin: ORIGIN };

  progress.log(`GET ${MODIIN_PATH}`);
  const pageHtml = await fetchText(MODIIN_URL, apiHeaders);
